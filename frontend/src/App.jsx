...
import addLog from "./utils/updateLogsHelper";

function App() {
  function changeFilter(newFilter) {
    setListFilter(newFilter);
    addLog("Set filter to: " + newFilter);
  }
  function toggleActiveStatus(extKey) {
    setAllExtensions((prevState) =>
      prevState.map((anExt, idx) => {
        if (extKey === idx) {
          addLog(
            `${anExt.isActive ? "Disabled" : "Enabled"} the ${
              anExt.name
            } extension!`
          );
  ...
  const filteredExtensions = useMemo(() => {
    return allExtensions.map(({ logo, name, description, isActive }, idx) => {
      if (
        !(listFilter === "Active" && isActive) &&
        !(listFilter === "Inactive" && !isActive) &&
        !(listFilter === "All")
      ) {
        return;
      }
      return (
        <ExtensionCard
          ...
          isActive={isActive}
          className={"w-full"}
          onClickToggleActive={() => toggleActiveStatus(idx)}
          onClickRemove={() => removeExt(name)}
        />
      );
    });
          ...
          <div className="flex gap-2">
            {["All", "Active", "Inactive"].map((elem, idx) => (
              <FilterButton
                key={idx}
                text={elem}
                isActive={elem === listFilter}
                isFilterButton={true}
                onClick={() => changeFilter(elem)}
              />
            ))}
          </div>
        </div>
